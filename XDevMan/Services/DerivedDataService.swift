...
actor DerivedDataService: DerivedDataServiceInterface {
        ...
        let derDataList: [DerivedData] = list.map { derivedData in
            ...
            case "Fleet":
                ...
                .map({ app in
                    ...
                    guard let realDR = ((try? fileManager.contentsOfDirectory(atPath: dr.path)) ?? [])
                        .filter({ $0.count > 29 && $0.dropFirst($0.count - 29).hasPrefix("-") }).first
                    else {
                        return []
                    }
                    let name = String(realDR.dropLast(29))
                    let appDerivedData = derivedData.path.appending(path: app).appending(path: "DerivedData")
                    return [
                        DerivedDataApp(
                            name: name,
                            path: appDerivedData.appending(path: realDR)
                        ),
                        DerivedDataApp(
