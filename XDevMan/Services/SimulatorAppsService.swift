...
actor SimulatorAppsService: SimulatorAppsServiceInterface {
    ...
    func apps(for sim: DeviceSim) async -> [SimAppItem] {
        ...
        if sims.isEmpty {
            sims = apps(shutdown: sim)
        }
        return sims.sorted(by: { $0.name < $1.name })
    }

    private func apps(shutdown sim: DeviceSim) -> [SimAppItem] {
        ...
        let apps = ((try? fileManager.contentsOfDirectory(atPath: appsDir.path)) ?? [])
            ...
            .compactMap({ path -> App? in
                ...
                guard let bundleId = dict["CFBundleIdentifier"] as? String,
                      ...
                      let displayName = (dict["CFBundleDisplayName"] as? String ?? dict["CFBundleName"] as? String)
                else {
                    return nil
                }
                var icon: URL?
